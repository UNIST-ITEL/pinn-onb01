...
ratio $w_{\mathrm{data}}/w_{\mathrm{cond}} \approx 17.8$ is by
deliberate design rather than an artefact of optimization. The
present PINN is intended as a \emph{physics-regularized regressor}, not
a forward conduction solver. The 1D conduction PDE and the McAdams
boundary closure act as soft inductive biases that anchor the network
to globally consistent thermodynamic behaviour, while the dataset
encodes the micro-scale cavity activation physics that the 1D PDE
cannot resolve. Strongly penalizing PDE residuals would force the
network to discard sub-grid variability that is essential for
predictive accuracy on engineered surfaces; the balance is therefore
chosen to enforce macroscopic conservation while allowing the data
term to capture the irreducible heterogeneity of incipience. The Hsu
hinge acts as soft consistency rather than a fitting target.
