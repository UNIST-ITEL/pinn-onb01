...
Stage 1 status (2026-05-19): SKELETON.
    Phase 1 source code is currently still located under the workspace
    root ``03_model/src/``. This package re-exports those modules
    transparently so that downstream phase code can already say
...
# --- Stage 1 shim: inject 03_model/src into sys.path so the legacy
# Phase 1 modules (model, loss, training, utils) become importable as
# regular top-level packages. Stage 2 will replace this with real code
# under pinn_onb/model, pinn_onb/loss, etc.
_WORKSPACE_ROOT = Path(__file__).resolve().parents[3]
_LEGACY_SRC = _WORKSPACE_ROOT / "03_model" / "src"
