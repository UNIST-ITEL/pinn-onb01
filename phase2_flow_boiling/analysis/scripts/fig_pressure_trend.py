...
def main() -> None:
    ...
    sweeps = [("wang2024", 1000, 2e5, "Wang 2024: $G{=}1000$, $q''{=}200$", "#1565C0", "o"),
              ("kuang2025", 500, 3e5, "Kuang 2024: $G{=}500$, $q''{=}300$", "#C62828", "s")]
    for pid, G, q, lab, c, mk in sweeps:
        ...
        ax.plot(d.P_kPa / 1000, d.pred, mk + "--", color=c, ms=6, mfc="white", lw=1.3,
                label=f"{lab} (predicted)")
    ax.set_xscale("log"); ax.set_xlabel("Pressure $P$ [MPa]"); ax.set_ylabel("$\\Delta T_{ONB}$ [K]")
    ax.legend(fontsize=8.5, loc="upper right")
    ax.text(0.03, 0.04,
            "corr($P$, predicted $\\Delta T$):  +0.01 (no $P$ feature) $\\rightarrow$ $-$0.69 (with $P$ feature);  measured $-$0.57",
