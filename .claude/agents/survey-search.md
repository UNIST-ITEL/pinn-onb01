...
1. **입력 확인**: 사용자 요청에서 키워드 추출. 없으면 갭 매트릭스(`01_survey/gap_matrix.md`)에서 ❌ 셀의 키워드를 우선 활용.
2. **검색 실행**:
...
4. **중복 제거**: `01_survey/paper_database.md`에 이미 있는 논문 제외.
5. **출력**:
   - 후보 논문 5~15편을 `01_survey/search_candidates_YYYY-MM-DD.md`에 표 형식으로 저장.
